...
import { getRoleTemplateById, updateRoleTemplateById } from 'apiSdk/role-templates';
...
function RoleTemplateEditPage() {
  ...
  const handleSubmit = async (values: RoleTemplateInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateRoleTemplateById(id, values);
      mutate(updated);
      resetForm();
